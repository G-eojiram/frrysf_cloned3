...
<template>
    <div id="marginright" class="lg:mx-[5rem] sm:mx-[1rem]">
        ...
        <div class="mt-[1rem] border-t py-[2rem] flex justify-end gap-5 text-sm font-medium px-[1rem]">
            <button
                @click.prevent = "responseRequest(route.params.id, 'Approved', 'TRUE');  refreshRoute($router)" 
                type="button" class=" border-gray-800 bg-gray-700 text-white hover:bg-gray-600 py-2 px-[4rem] rounded-md shadow-md">
                Approve
            </button>
            <button
                @click.prevent = "responseRequest(route.params.id, 'Rejected', 'FALSE'),  refreshRoute($router)" 
                type="button" class="border-gray-800 bg-gray-200 text-gray-500 hover:bg-gray-100 py-2 px-[4rem] rounded-md shadow-md">
                Reject
            </button>
        </div>
    </div>
